...
const upsertPriceRecord = async(price: Stripe.Price) => {
  const priceData: Price = {
    ...
    trial_period_days: price.recurring?.interval_count,
...
console.log(`Price inserted|updated: ${price.id}`);
  ...
  const manageSubscriptionStatusChange = async(subscriptionId: string, customerId: string, createAction = false) => {
    ...
    const {data: customersData, error: noCustomerError} = await supabaseAdmin.from('customers').select('id').eq('stripe_customer_id', customerId).single();

    if(noCustomerError) throw noCustomerError;

    const {id: uuid } = customersData!;
